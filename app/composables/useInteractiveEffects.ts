...
function initCounters() {
  const counters = Array.from(document.querySelectorAll<CounterEl>(".counter"));
...
function initParallax() {
  ...
  function onMove(e: MouseEvent) {
    ...
    (
      img as HTMLElement
    ).style.transform = `translate3d(0,0,0) rotateX(${ry}deg) rotateY(${rx}deg)`;
  }
...
function initMagnetic() {
  ...
  mags.forEach((el) => {
    const targetEl = (el.querySelector(".magnetic-inner") as HTMLElement) || el;
...
function initMagneticDepth() {
  ...
  els.forEach((el) => {
    ...
    function onMove(e: MouseEvent) {
      ...
      const strengthEdge = Math.max(Math.abs(x - 0.5), Math.abs(y - 0.5)) * 2;
    ...
    function onLeave() {
      ...
      target = { rx: 0 + rx, ry: 0 + ry, tx: 0 + tx, ty: 0 + ty, scale: 1 };
      if (!raf)
  ...
  return () =>
    els.forEach((el) => {
      const inner = (el.querySelector(".magnetic-inner") as HTMLElement) || el;
...
function initParticles() {
  ...
  function step() {
    ...
    for (const p of particles) {
      ...
      (ctx as CanvasRenderingContext2D).fillStyle = "rgba(0,200,255,0.55)";
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 130 * 130) {
          ...
          (ctx as CanvasRenderingContext2D).strokeStyle = `rgba(0,180,255,${
            o * 0.25
          })`;
...
export function useInteractiveEffects() {
  ...
  onMounted(() => {
    ...
    document.documentElement.dataset.magneticStrength = String(baseStrength);
    ...
    const maybe = [
      ...
      prefersReduced || !particlesEnabled.value || performanceReduced.value
        ? undefined
        : initParticles(),
      prefersReduced ? undefined : initParallax(),
      prefersReduced ? undefined : initTilt(),
      prefersReduced ? undefined : initMagnetic(),
